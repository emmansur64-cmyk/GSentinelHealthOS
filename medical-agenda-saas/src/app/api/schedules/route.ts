...
export async function GET(request: Request) {
  ...
  const rules = await prisma.availabilityRule.findMany({
    where: doctorId ? { doctor_id: doctorId } : undefined,
...
export async function POST(request: Request) {
  const authUser = await getAuthenticatedUser();
  if (!authUser) return fail("No autenticado", 401);
  if (!hasRole(authUser, ["secretaria"])) return fail("Sin permisos", 403);
  ...
  try {
    ...
    const doctor = await prisma.doctorProfile.findUnique({ where: { user_id: parsed.data.doctor_id } });
