...
export class MedicalAssistantService {
  ...
  async handleWhatsAppMessage(input: MedicalAssistantRequest): Promise<MedicalAssistantResponse> {
    ...
    const roleClassification = roleHint
      ...
      : this.aiService.classifyMedicalRole(query || 'consulta medica por whatsapp');
    ...
    if (hasText) {
      const incident: IncidentPayload = {
        id: `whatsapp-${Date.now()}-${randomUUID().slice(0, 8)}`,
        source: 'whatsapp-medical-assistant',
        message: query,
